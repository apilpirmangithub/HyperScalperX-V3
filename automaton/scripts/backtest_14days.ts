...
const START_TIME = END_TIME - (14 * DAY_MS);
...
async function runBacktest() {
    ...
    console.log(`Period: Last 14 Days (${new Date(START_TIME).toLocaleDateString()} - ${new Date(END_TIME).toLocaleDateString()})`);
    ...
    const report = `
# 👑 HYPERSCALPERX BACKTEST REPORT (LAST 14 DAYS) 👑
...
`;
    ...
    console.log(report);
    fs.writeFileSync("backtest_14days_report.md", report);
}
