...
const START_TIME = END_TIME - (21 * DAY_MS);
...
async function runBacktest() {
    ...
    console.log(`Period: Last 21 Days (${new Date(START_TIME).toLocaleDateString()} - ${new Date(END_TIME).toLocaleDateString()})`);
    ...
    const report = `
# 👑 HYPERSCALPERX BACKTEST REPORT (LAST 21 DAYS) 👑
...
`;
    ...
    console.log(report);
    fs.writeFileSync("backtest_21days_report.md", report);
}
