...
const START_TIME = END_TIME - (60 * DAY_MS);
...
async function runBacktest() {
    console.log("🚀 HYPERSCALPERX - FULL 60-DAY OPTIMIZED BACKTEST 🚀");
    console.log(`Period: Last 60 Days (${new Date(START_TIME).toLocaleDateString()} - ${new Date(END_TIME).toLocaleDateString()})`);
    console.log("------------------------------------------");
    ...
    for (const asset of assets) {
        assetData[asset] = await fetchCandlesChunked(info, asset, START_TIME - (10 * DAY_MS), END_TIME);
        console.log(`✅ Loaded ${asset} (${assetData[asset].length} candles)`);
    }
    ...
    for (const t of sortedTs) {
        if (activePos) {
            ...
            if (c) {
                ...
                if (isLong ? c.l <= slPrice : c.h >= slPrice) exit = "SL";
                else if (activePos.peak >= CONFIG.trailStart && pnlPct < activePos.peak - CONFIG.trailCallback) exit = "TP";
    ...
    const report = `
# 🚀 FINAL RESULT: 60-DAY SULTAN EVOLUTION BACKTEST
### Analisis Performa Strategi Terkunci (Z-3.2, Margin 50%)
...
### 💡 Kesimpulan Akhir:
Strategi ini telah teruji secara statistik selama 2 bulan terakhir. Meskipun agresif, filter Z-Score 3.2 memberikan perlindungan yang sangat baik terhadap fluktuasi pasar yang tidak berarti.
`;

    console.log(report);
    fs.writeFileSync("backtest_optimized_60days.md", report);
}
