...
async function runBacktest() {
    console.log("🚀 HYPERSCALPERX - OPTIMIZED BACKTEST (7 DAYS) 🚀");
    ...
    const report = `
# 🚀 RESULT: OPTIMIZED SULTAN BACKTEST (7 DAYS)
### Performa dengan Parameter Baru
...
### 📈 Trade Log:
${tradeHistory.map(h => `- **${h.asset}** ${h.dir}: ${h.pnlPct.toFixed(2)}% via ${h.reason}`).join("\n")}
`;

    console.log(report);
    fs.writeFileSync("backtest_optimized_7days.md", report);
}
